...
def send_graph(G, gs, rgb=(1,0,0), label_fcn=str):
    '''
    ...
    gnodes = make_gephi_nodes(G.nodes(), rgb=rgb, size=10, label_fcn=label_fcn)

    for k,v1 in gnodes.items():
        gs.add_node(v1)
        try:
            for v2 in make_gephi_nodes(G[k].keys(), label_fcn=label_fcn).values():  # G[k] is networkx node neighbor dictionary
                gs.add_node(v2)
